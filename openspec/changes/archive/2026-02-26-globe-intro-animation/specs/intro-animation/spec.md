...
地圖 SHALL 在初始化期間顯示白色全屏 overlay，覆蓋 MapLibre 載入前的空白期。overlay SHALL 在動畫開始時（或無 intro 時在地圖 ready 時）以 400ms CSS opacity transition 淡出。
...
- **THEN** overlay 以 400ms 淡出，發送 READY

#### Scenario: 有 intro 時 overlay 與動畫同步
- **WHEN** intro 啟用，MapLibre 地圖載入完成
- **THEN** overlay 開始淡出的同時，intro 動畫開始播放

### Requirement: 地球進場動畫（intro）
地圖 SHALL 支援可選的開場動畫：從低 zoom、偏移的起始位置，以 `map.easeTo()` 平滑旋轉並縮近至目標 center 與 zoom。

#### Scenario: 動畫從偏移位置縮近至目標
- **WHEN** intro 啟用，地圖 ready
- **THEN** 地圖從 zoom=1、center=[targetLng - introRotate, 0] 開始，easeTo 至目標 center 與 zoom
...
地圖 SHALL 接受 `{ type: 'PLAY_INTRO' }` inbound 訊息，重新播放完整 intro 動畫序列（overlay 淡入 → 重置相機 → easeTo → READY）。

#### Scenario: 重播 intro 動畫
- **WHEN** 外部網站在地圖已載入後發送 `{ type: 'PLAY_INTRO' }`
- **THEN** overlay 淡入，相機重置至起始位置，然後動畫至目標，完成後發送 READY

### Requirement: introDuration 控制動畫時長
地圖 SHALL 支援 `introDuration` URL 參數（毫秒，預設 3000）控制 easeTo 的動畫時長。

#### Scenario: 自訂動畫時長
- **WHEN** URL 含 `?intro=true&introDuration=5000`
- **THEN** easeTo 動畫持續 5 秒

#### Scenario: 使用預設時長
- **WHEN** URL 含 `?intro=true` 但未指定 introDuration
- **THEN** easeTo 動畫持續 3 秒（預設值）

### Requirement: introRotate 控制起始偏移角度
地圖 SHALL 支援 `introRotate` URL 參數（度數，預設 90）控制動畫起始點相對於目標 longitude 的偏移量。
